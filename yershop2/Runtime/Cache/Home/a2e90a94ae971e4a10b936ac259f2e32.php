...
<dl><dt><a rel="nofollow">文章分类</a></dt>
<?php if(is_array($article)): $i = 0; $__LIST__ = $article;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><dd><a rel="nofollow"  href="<?php echo U('help/lists?pid='.$vo['id']);?>"><?php echo ($vo["title"]); ?></a></dd><?php endforeach; endif; else: echo "" ;endif; ?>
</dl>
<?php if(is_array($footer)): $i = 0; $__LIST__ = $footer;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><dl><dt><a href="<?php echo U('help/lists?pid='.$vo['id']);?>"><?php echo ($vo["title"]); ?></a></dt>
...
<dl class='artlist'>
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><dd><a  href="<?php echo U('help/detail?id='.$vo['id']);?>"><?php echo ($vo["title"]); ?></a>(<?php echo (date('Y-m-d H:i',$vo["create_time"])); ?>)</dd><?php endforeach; endif; else: echo "" ;endif; ?>
</dl>			
