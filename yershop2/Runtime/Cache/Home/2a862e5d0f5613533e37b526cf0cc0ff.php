...
购买人数：<?php echo ($info["sale"]); ?>人已团</li>

<li>剩余时间： <?php date_default_timezone_set("Asia/Shanghai"); $time1 = $info[deadline]; $time2 =time(); $second = $time1 - $time2; $day = floor($second/(3600*24)); $second = $second%(3600*24); $hour = floor($second/3600); $second = $second%3600; $minute = floor($second/60); $second = $second%60; if($second>0){echo $day.'天'.$hour.'小时'.$minute.'分'.$second.'秒'; } else{echo '0天0小时0分0秒'; } ?>
</li>
...
<?php date_default_timezone_set("Asia/Shanghai"); $time1 = $info[deadline]; $time2 =time(); $second = $time1 - $time2; ?>
 <!-- 根据计算的时间显示文字 -->
<?php if(($second) > "0"): ?><a rel="nofollow" href="javascript:void(0)"  onclick='order(<?php echo ($info["id"]); ?>);return false;' class="dginfo_btn">立刻抢购</a>
<a  rel="nofollow" href="javascript:void(0)" onclick='buy(<?php echo ($info["id"]); ?>);return false;' class="dginfo_btn"  >加入购物车</a><?php else: ?><a rel="nofollow" href="javascript:void(0)"   class="dginfo_btn" style='background-color:#ccc;color:#fff'>已结束</a><?php endif; ?>
