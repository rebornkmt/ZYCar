...
<?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cate): $mod = ($i % 2 );++$i;?><div class="item">
   ...
   <section>
	  <div class="tuanlist">
       <?php if(empty($childlist)): else: ?> <div class="category_nav">
      <dl>
      <dt><a href="<?php echo U('Tuan/index');?>">全部分类</a></dt>
      
      <dd><?php if(is_array($childlist)): $i = 0; $__LIST__ = $childlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a href="<?php echo U('Article/lists/?category='.$vo['name']);?>"><?php echo ($vo["title"]); ?>(<?php echo (get_tuan_count($vo["id"])); ?>)</a><?php endforeach; endif; else: echo "" ;endif; ?> </dd></dl> 
       </div><?php endif; ?>   
        <ul class="ml_content_top"><li <?php if(($order) == "1"): ?>class="active"<?php else: endif; ?>><a  href="<?php echo U('Article/index',array(pid=>$category['id'],order=>1,'sort'=>$see));?>" class="<?php echo ($value); ?>">热度<i></i></a></li><li <?php if(($order) == "2"): ?>class="active"<?php else: endif; ?>><a  href="<?php echo U('Article/index',array(pid=>$category['id'],order=>2,'sort'=>$see));?>"  class="<?php echo ($value); ?>">最新<i></i></a></li><li <?php if(($order) == "3"): ?>class="active"<?php else: endif; ?>><a  href="<?php echo U('Article/index',array(pid=>$category['id'],order=>3,'sort'=>$see));?>"  class="<?php echo ($value); ?>">价格<i></i></a></li><li <?php if(($order) == "4"): ?>class="active"<?php else: endif; ?>><a  href="<?php echo U('Article/index',array(pid=>$category['id'],order=>4,'sort'=>$see));?>"  class="<?php echo ($value); ?>">销量<i></i></a></li></ul> 
                <?php if(empty($list)): ?><div class="media">暂无数据</div><?php endif; ?>
    
                <div class="yershop_tuan">


                <ul> <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="li" onmouseover="this.className='lihover'" onmouseout="this.className='li'"><span class="icon_discount"></span>
               <a href="<?php echo U('Article/detail?id='.$vo['id']);?>"><img src="<?php echo (get_cover($vo["cover_id"],'path')); ?>"> </a> 
               <p class="title"><?php echo ($vo["title"]); ?></p>
			    <p><span class="tuan"><strong>￥<?php echo ($vo["price"]); ?></strong>/<strong class="yuan">￥<?php echo (get_marketprice($vo["id"])); ?></strong></span><span class="qiang"><a>去抢购</a></span></p>
				  <p class="time">剩余时间：<?php date_default_timezone_set("Asia/Shanghai"); $time1 = $vo[deadline]; $time2 =time(); $second = $time1 - $time2; $day = floor($second/(3600*24)); $second = $second%(3600*24); $hour = floor($second/3600); $second = $second%3600; $minute = floor($second/60); $second = $second%60; if($second>0){echo $day.'天'.$hour.'小时'.$minute.'分'.$second.'秒'; } else{echo '0天0小时0分0秒'; } ?> </p>
...
<?php echo ($page); ?>
     ...
     <script>
    $(".menu-list li").eq(1).css('background','#d25400');  
     </script>   
