...
<link href="/yershop2/Public/Home/css/detail.css" rel="stylesheet">

<link href="/yershop2/Public/Home/css/footer.css" rel="stylesheet">
<!-- 评论统计工具 -->
<link href="/yershop2/Public/Home/css/jquery.circliful.css" rel="stylesheet" type="text/css" />
...
<div id="zoom">
	...
	<div id="spec-n5">
		<div class="control" id="spec-left">
			<img src="/yershop2/Public/Home/images/btn_mobile_prev_cur.jpg" />
		</div>
		<div id="spec-list">
  ...
  <li class="img"><img src="<?php echo (get_cover($info["cover_id"],'path')); ?>" height="50" width="50"/></li>
   <?php if(!empty($pics)): if(is_array($pics)): $i = 0; $__LIST__ = $pics;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><li><img src="/yershop2<?php echo (get_cover($v,'path')); ?>" data-id="<?php echo ($v); ?>"/> </li><?php endforeach; endif; else: echo "" ;endif; endif; ?> 
		<?php if(empty($pics)): ?><li><img src="/yershop2/Public/Home/images/2-1.jpg"> </li>
				<li><img src="/yershop2/Public/Home/images/2-2.jpg"> </li>
					
			<li class="img"><img src="<?php echo (get_cover($info["cover_id"],'path')); ?>" height="50" width="50"/></li>
			<li><img src="/yershop2/Public/Home/images/2-3.jpg"> </li>
		<li><img src="/yershop2/Public/Home/images/2-4.jpg"> </li><?php endif; ?>
			</ul>
		</div>
		<div class="control" id="spec-right">
			<img src="/yershop2/Public/Home/images/btn_mobile_next_cur.jpg" />
		</div>
    </div>
</div><!-- wwww over--> <!-- 商品参数-->
...
<p class=""><span class="quantity">数量：</span><span class="quantity-form"><a href="javascript:void(0)" class="jia"  onclick="add()" >+</a>  
 ...
 <?php if(!empty($info['color'])): ?><li class="colorid pp_prop_wrap">颜色： <?php if(!empty($info['color'])){ $colorlist= explode('、',$info['color']);}else{ $colorlist="";} ?>
 <?php if(!empty($colorlist)): if(is_array($colorlist)): $k = 0; $__LIST__ = $colorlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k % 2 );++$k;?><a href="javascript:void(0)" class="<?php if($k == 1): ?>cur<?php else: endif; ?>" onclick='setcolor(<?php echo ($k); ?>);return false;'"><?php echo ($v); ?> </a><?php endforeach; endif; else: echo "" ;endif; endif; ?></li><?php endif; ?> 
 ...
 <?php if(!empty($datalist)): if(is_array($datalist)): $k = 0; $__LIST__ = $datalist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k % 2 );++$k;?><a href="javascript:void(0)" class="<?php if($k == 1): ?>cur<?php else: endif; ?>" onclick='setprice(<?php echo ($k); ?>);return false;'"><?php echo ($v); ?> </a><?php endforeach; endif; else: echo "" ;endif; endif; ?></li><?php endif; ?> 

<script>
function setcolor(num){
str=null;
var str='<?php echo ($info["color"]); ?>';
var value=str.split("、");
var i=num-1;
var str=$(".weight").find(".cur").text()+$(".colorid a").eq(i).text();
$("#inputparameters").val(str);
$(".colorid a").eq(i).addClass("cur").siblings().removeClass("cur");
}
...
var str=$(".weight a").eq(i).text()+$(".colorid ").find(".cur").text();
$("#inputparameters").val(str);
...
<!-- buy-->
<p>
<span>收藏产品：<a href="javascript:vod(0);" onclick="favor();return false;"><img title="收藏按钮" src="/yershop2/Public/Home/images/icon_favorite_pro.jpg" ></a>
</span>
&nbsp;&nbsp;&nbsp;&nbsp;<span>&nbsp;&nbsp;&nbsp;分享到：<a title="分享到新浪微博" target="_blank" href="http://service.t.sina.com.cn/share/share.php?title=<?php echo ($info["title"]); ?>。（来自<?php echo C('SITENAME');?>）&amp;url=<?php echo C('DOMAIN'); echo U('Article/detail?id='.$info['id']);?>&amp;pic=<?php echo C('DOMAIN'); echo (get_cover($info["cover_id"],'path')); ?>" data-item="sina" class="J_vivo_share">
            <img alt="新浪微博" src="/yershop2/Public/Home/images/icon_sina_weibo.jpg" app="b2c">        </a>                                                                
        <a title="分享到腾讯微博" target="_blank" href="http://v.t.qq.com/share/share.php?title=<?php echo ($info["title"]); ?>（来自<?php echo C('SITENAME');?>）。&amp;url=<?php echo C('DOMAIN'); echo U('Article/detail?id='.$info['id']);?>&amp;pic=<?php echo C('DOMAIN'); echo (get_cover($info["cover_id"],'path')); ?>" data-item="tencent" class="J_vivo_share">
            <img alt="腾讯微博" src="/yershop2/Public/Home/images/icon_tencent_weibo.jpg" app="b2c">        </a>                                                                
        <a target="_blank" title="分享到人人网" href="http://widget.renren.com/dialog/share?resourceUrl=<?php echo C('DOMAIN'); echo U('Article/detail?id='.$info['id']);?>;srcUrl=<?php echo C('DOMAIN');?>&amp;title=<?php echo ($info["title"]); ?>。（来自<?php echo C('SITENAME');?>）&amp;pic=<?php echo C('DOMAIN'); echo (get_cover($info["cover_id"],'path')); ?>" data-item="renren" class="J_vivo_share">
            <img alt="人人网" src="/yershop2/Public/Home/images/icon_renrenwang.jpg" app="b2c">        </a>           
</span></p>
...
<SCRIPT src="/yershop2/Public/static/js/base.js" type=text/javascript></SCRIPT>
...
<SCRIPT src="/yershop2/Public/static/js/jqueryzoom-jcarousel.js" type="text/javascript"></SCRIPT>
<!-- js over-->
<!-- jqzoom over-->
<!-- 商品参数-->
...
<div class="union"><form action="<?php echo U("order/add");?>"  class="group_buy"name="fiveform" id="orform" method="post" onsubmit="return trySubmit()">
...
<input type="hidden" name="sort[]"  value="<?php echo ($v); ?>"/>	<input type="hidden"  class="goodnum"   name="num[]" value="1"/> <input class="ids row-selected"  checked="" type="checkbox" name="id[]" value="<?php echo ($v); ?>">
<span class="d_price">￥<?php echo (get_good_price($v)); ?></span></p></div>
	<!-- 判断是否是最后一个组合商品-->

	
	<input type="hidden"  class="goodnum"   name="num[]"value="<?php echo ((isset($vo["num"]) && ($vo["num"] !== ""))?($vo["num"]):'1'); ?>"/>
...
<div id="goods_content">
	 ...
	 <P>操作系统 ：<?php echo ((isset($info["system"]) && ($info["system"] !== ""))?($info["system"]):"无"); ?></P>
  <P>网络制式 ：<?php echo ((isset($info["nettype"]) && ($info["nettype"] !== ""))?($info["nettype"]):"无"); ?></P>
   <P>屏幕尺寸 ：<?php echo ((isset($info["screen"]) && ($info["screen"] !== ""))?($info["screen"]):"无"); ?></P>
  <P>颜色 ：<?php echo ((isset($info["color"]) && ($info["color"] !== ""))?($info["color"]):"无"); ?></P>	
   <P>上市时间：<?php echo (date("Y-m-d",$info["launch-time"])); ?></P>	
   <P>RAM容量 ：<?php echo ((isset($info["ram"]) && ($info["ram"] !== ""))?($info["ram"]):"无"); ?></P>
 <P>ROM容量 ：<?php echo ((isset($info["rom"]) && ($info["rom"] !== ""))?($info["rom"]):"无"); ?></P>
  <P>cpu型号：<?php echo ((isset($info["cpu_type"]) && ($info["cpu_type"] !== ""))?($info["cpu_type"]):"无"); ?></P>
   <P>cpu频率：<?php echo ((isset($info["cpu_frequency"]) && ($info["cpu_frequency"] !== ""))?($info["cpu_frequency"]):"无"); ?></P>
  <P>电池类型 ：<?php echo ((isset($info["battery_type"]) && ($info["battery_type"] !== ""))?($info["battery_type"]):"无"); ?></P>	
    <P>电池容量 ：<?php echo ((isset($info["battery_capacity"]) && ($info["battery_capacity"] !== ""))?($info["battery_capacity"]):"无"); ?></P>
   <P>待机时间：<?php echo ((isset($info["stand-by-time"]) && ($info["stand-by-time"] !== ""))?($info["stand-by-time"]):"无"); ?></P>
  <P>通话时间 ：<?php echo ((isset($info["holding-time"]) && ($info["holding-time"] !== ""))?($info["holding-time"]):"无"); ?></P>
  <P>手机重量 ：<?php echo ((isset($info["phoneweight"]) && ($info["phoneweight"] !== ""))?($info["phoneweight"]):"无"); ?></P>
     <P>扩展容量 ：<?php echo ((isset($info["sdcard"]) && ($info["sdcard"] !== ""))?($info["sdcard"]):"无"); ?></P>
   <P>功能特点：<?php echo ((isset($info["character"]) && ($info["character"] !== ""))?($info["character"]):"无"); ?></P>
  <P>摄像头类型 ：<?php echo ((isset($info["camera-type"]) && ($info["camera-type"] !== ""))?($info["camera-type"]):"无"); ?></P>
  <P>后置摄像头像素 ：<?php echo ((isset($info["camera-after"]) && ($info["camera-after"] !== ""))?($info["camera-after"]):"无"); ?></P>	
 <P>前置摄像头像素 ：<?php echo ((isset($info["camera-before"]) && ($info["camera-before"] !== ""))?($info["camera-before"]):"无"); ?></P>	
  <P>传感器类型 ：<?php echo ((isset($info["sensor-type"]) && ($info["sensor-type"] !== ""))?($info["sensor-type"]):"无"); ?></P>	
   <P>闪光灯 ：<?php echo ((isset($info["flashlight"]) && ($info["flashlight"] !== ""))?($info["flashlight"]):"无"); ?></P>	
   <P>光圈：<?php echo ((isset($info["apeture"]) && ($info["apeture"] !== ""))?($info["apeture"]):"无"); ?></P>	
...
<ul class="item_wrap" id="commentboxmiddle">
    <?php if(is_array($listmiddle)): $i = 0; $__LIST__ = $listmiddle;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><!--内容输出-->
      <li class="comment_items clearfix" style="border-top:none">    
<div class="items_left">            
<div class="items_left_pic">               
<a target="_blank" title="<?php echo (get_regname($list["uid"])); ?>" ><img src="<?php echo (get_cover(get_face($list["uid"]),'path')); ?>"></a>                <span class="user_level">V3</span>             </div>            <p class="items_left_name" title="<?php echo (get_regname($list["uid"])); ?>"><?php echo (get_regname($list["uid"])); ?>" ></p>             <p class="items_left_level">钻石会员</p>         </div>         <div class="items_right">             <div class="items_detail">                 <p class="describe_detail">                     </p><p class="describe_detail"><?php echo ($list["content"]); ?></p><p></p>                              </div>             <div class="items_user">              
...
<?php echo (date('Y-m-d H:i',$list["create_time"])); ?></span>                     <span class="items_tips fr">                                                                  </span>                      </div>                                               </div>         </div>     </li><?php endforeach; endif; else: echo "无" ;endif; ?>
  ...
  <!--精彩评论tabs-->  
<script src="/yershop2/Public/Home/js/jquery.idTabs.min.js"></script>
	<script type="text/javascript"> 
  $(".comment_tabs").idTabs();
</script>
  <!--精彩评论tabs-->  
 ...
 <div id="header4" class="choose" style="display:none">
   ...
   <div class="item_wrap" id="boxquest">
	   <?php if(is_array($listquestion)): $i = 0; $__LIST__ = $listquestion;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$m): $mod = ($i % 2 );++$i;?><div class="qa_cont clearfix">
					...
					<span><img src="/yershop2/Public/Home/images/question.png">问：<?php echo ($m["content"]); ?>？</span></div>   
		
		  	 
		 <?php if(is_array($m['id'])): $i = 0; $__LIST__ = $m['id'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$re): $mod = ($i % 2 );++$i;?><div class="answer"><span class="time"><?php echo (date('Y-m-d H:i',$re["create_time"])); ?></span><span><b><img src="/yershop2/Public/Home/images/anwser.png">商家：<?php echo ($re["content"]); ?></b></span></div><?php endforeach; endif; else: echo "" ;endif; ?>  
		   </div><?php endforeach; endif; else: echo "" ;endif; ?>
...
<?php echo ($pagequestion); ?>
  ...
  $.post("<?php echo U('service/addmessage');?>", {
    ...
    var html='<div class="qa_cont clearfix"><div class="question"><span class="time">'+data.time+'</span><span><img src="/yershop2/Public/Home/images/question.png">问：'+str+'？</span></div>            </div>';
...
if($(".weight a").length>0&&$(".colorid a").length>0){
var s=$(".weight .cur").index()+1;
var b=$(".colorid .cur").index()+1;
var string=String(gid)+String(s)+String(b);
}
if($(".weight a").length>0&&$(".colorid a").length<1){
var s=$(".weight .cur").index()+1;
var string=String(gid)+String(s);
}
if($(".weight a").length<1&&$(".colorid a").length<1){
var string=String(gid);
}
$("#inputsort").text(string);
$("#inputparameters").val($(".colorid .cur").text()+$(".weight .cur").text());
...
var parameters=$(".colorid .cur").text()+$(".weight .cur").text();//参数
var s=$(".weight").find(".cur").index()+1;
var b=$(".colorid").find(".cur").index()+1;
var string=String(gid)+String(s)+String(b);
$.ajax({
type:'post', //传送的方式,get/post
