...
<p >价格：<span class="dginfo_price">￥<em class="price" id="resetprice"><?php if(!empty($info['groupprice'])){ $price= explode('、',$info['groupprice']);}else{ $price="";} ?>
 <?php if(!empty($price)): if(is_array($price)): $k = 0; $__LIST__ = array_slice($price,0,1,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k % 2 );++$k; echo ($v); endforeach; endif; else: echo "" ;endif; endif; ?>
		<?php if(empty($info['groupprice'])): echo ($info["price"]); endif; ?></em></span></p>
...
<input type="hidden" name="sort[]" id="inpusort"  value="<?php echo ($info["id"]); ?>"/> 
...
<?php if(!empty($info['color'])): ?><li class="colorid pp_prop_wrap">颜色： <?php if(!empty($info['color'])){ $colorlist= explode('、',$info['color']);}else{ $colorlist="";} ?>
 <?php if(!empty($colorlist)): if(is_array($colorlist)): $k = 0; $__LIST__ = $colorlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k % 2 );++$k;?><a href="javascript:void(0)" class="<?php if($k == 1): ?>cur<?php else: endif; ?>" title="<?php echo ($v); ?>" onclick='setcolor(<?php echo ($k); ?>);return false;'"><?php echo ($v); ?> </a><?php endforeach; endif; else: echo "" ;endif; endif; ?></li><?php endif; ?> 

  <?php if(!empty($info['size'])): ?><li class="weight pp_prop_wrap">尺码： <?php if(!empty($info['size'])){ $datalist= explode('、',$info['size']);}else{ $datalist="";} ?>
...
<input type="hidden" name="sort[]" value="<?php echo ($v); ?>"/>	
<input type="hidden"  class="goodnum"   name="num[]" value="1"/> 
<input class="ids row-selected"  checked="" type="checkbox" name="id[]" value="<?php echo ($v); ?>">
...
<div id="goods_content">
       <div id="header1" class="choose"><?php echo ($info["content"]); ?></div>
		<div id="header2" class="choose" style="display:none">
		 <P>品牌 ：<?php echo ((isset($info["brand"]) && ($info["brand"] !== ""))?($info["brand"]):"无"); ?></P>
  <P>尺码 ：<?php echo ((isset($info["size"]) && ($info["size"] !== ""))?($info["size"]):"无"); ?></P>
   <P>材质 ：<?php echo ((isset($info["material"]) && ($info["material"] !== ""))?($info["material"]):"无"); ?></P>
  <P>服装类型 ：<?php echo ($info["c_type"]); ?></P>
 <P>季节 ：<?php echo ((isset($info["season"]) && ($info["season"] !== ""))?($info["season"]):"无"); ?></P>
  <P>颜色 ：<?php echo ((isset($info["color"]) && ($info["color"] !== ""))?($info["color"]):"无"); ?></P>
  <P>款式 ：<?php echo ((isset($info["style"]) && ($info["style"] !== ""))?($info["style"]):"无"); ?></P>
		 </div>
...
var parameters=$(".weight .cur").text()+$(".colorid .cur").text();//参数
if($(".weight a").length>0&&$(".colorid a").length>0){
var s=$(".weight .cur").index()+1;
var b=$(".colorid .cur").index()+1;
var string=String(gid)+String(s)+String(b);
