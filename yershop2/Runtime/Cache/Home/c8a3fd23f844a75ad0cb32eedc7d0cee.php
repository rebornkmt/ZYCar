...
.times{height:30px; width:320px; background:url(images/clock.jpg) no-repeat; padding-left:0px}
.times p{height:20px; line-height:20px;}
.endtime{font-size:14px; font-family:"Microsoft Yahei"; color:#000}
                ...
                <ul> <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="li" onmouseover="this.className='lihover'" onmouseout="this.className='li'"><span class="icon_qg"></span>
               <a href="<?php echo U('Article/detail?id='.$vo['id']);?>"><img src="<?php echo (get_cover($vo["cover_id"],'path')); ?>"> </a> 
               <p class="title"><?php echo ($vo["title"]); ?></p>
			    <p><span class="tuan"><strong>￥<?php echo ($vo["price"]); ?></strong>/<strong class="yuan">￥<?php echo ($vo["marketprice"]); ?></strong></span><span class="qiang"><a>去抢购</a></span></p>
		

 <div class="times">
       
        剩余时间：<span class="endtime" value="<?php echo ($vo["deadline"]); ?>"></span>
     </div>
