...
<body>
 ...
 <?php if(isset($cate_rows)): ?></div><?php endif; ?> 
	...
	<div class="main-title">
		<h2>回复管理 [ <?php if(isset($_GET['group'])): ?><a href="<?php echo U('index');?>">全部</a><?php else: ?><strong>全部</strong><?php endif; ?>&nbsp;<?php if(is_array($group)): foreach($group as $key=>$vo): if(($group_id) != $key): ?><a href="<?php echo U('index?group='.$key);?>"><?php echo ($vo); ?></a><?php else: ?><strong><?php echo ($vo); ?></strong><?php endif; ?>&nbsp;<?php endforeach; endif; ?> ]</h2>
	</div>
	...
	<div class="cf">
        ...
        <div class="search-form fr cf">
            <div class="sleft">
                ...
                <a class="sch-btn" href="javascript:;" id="search" url="/yershop2/index.php?s=/Admin/Reply/index.html"><i class="btn-search"></i></a>
            </div>
        </div>
	</div>
	...
	<div class="data-table table-striped">
		 <table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$reply): $mod = ($i % 2 );++$i;?><tr>
					<td><input class="ids row-selected" type="checkbox" name="id[]" value="<?php echo ($reply["id"]); ?>"></td>
						<td><?php echo ($reply["id"]); ?></td>
						<td><?php echo ($reply["messageid"]); ?></td>
						<td><a title="编辑" href="<?php echo U('edit?id='.$reply['id']);?>"><?php echo (get_regname($reply["uid"])); ?></a></td>
						 
						 <td><?php echo ($reply["uid"]); ?></td>
						 <td><?php echo ($reply["content"]); ?></td>
						<td><?php echo ($reply["group"]); ?></td>
					   <td><?php echo ($reply["status"]); ?></td>
                         <td><?php echo (date('Y-m-d H:i:s',$reply["create_time"])); ?></td>
					<td>
                            <a title="编辑" href="<?php echo U('edit?id='.$reply['id']);?>">编辑</a>
                            <a class="confirm ajax-get" title="删除" href="<?php echo U('del?id='.$reply['id']);?>">删除</a>
                        </td>
