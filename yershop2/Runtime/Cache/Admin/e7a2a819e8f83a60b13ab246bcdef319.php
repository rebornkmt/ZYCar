...
<body>
 ...
 <?php if(isset($cate_rows)): ?></div><?php endif; ?> 
	...
	<div class="main-title">
		<h2>短信管理 [ <?php if(isset($_GET['group'])): ?><a href="<?php echo U('index');?>">全部</a><?php else: ?><strong>全部</strong><?php endif; ?>&nbsp;<?php if(is_array($group)): foreach($group as $key=>$vo): if(($group_id) != $key): ?><a href="<?php echo U('index?group='.$key);?>"><?php echo ($vo); ?></a><?php else: ?><strong><?php echo ($vo); ?></strong><?php endif; ?>&nbsp;<?php endforeach; endif; ?> ]</h2>
	</div>
	...
	<div class="cf">
        ...
        <div class="search-form fr cf">
            <div class="sleft">
                ...
                <a class="sch-btn" href="javascript:;" id="search" url="/yershop2/index.php?s=/Admin/Sms/index.html"><i class="btn-search"></i></a>
            </div>
        </div>
	</div>
	...
	<div class="data-table table-striped">
		 <table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sms): $mod = ($i % 2 );++$i;?><tr>
					<td><input class="ids row-selected" type="checkbox" name="id[]" value="<?php echo ($sms["id"]); ?>"></td>
						<td><?php echo ($sms["id"]); ?></td>
						<td><a title="编辑" href="<?php echo U('edit?id='.$sms['id']);?>"><?php echo ($sms["mobile"]); ?></a></td>
						 <td><?php echo ($sms["content"]); ?></td>	 
						 <td><?php echo ($sms["extcode"]); ?></td>
						  <td><?php echo ($sms["tempid"]); ?></td>
						  <td><?php echo ($sms["group"]); ?></td>	 
						  <td><?php echo ($sms["uid"]); ?></td>
                        <td><?php echo ($sms["status"]); ?></td>
                         <td><?php echo (date('Y-m-d H:i:s',$sms["create_time"])); ?></td>
					<td>
                            <a title="编辑" href="<?php echo U('edit?id='.$sms['id']);?>">编辑</a>
                            <a class="confirm ajax-get" title="删除" href="<?php echo U('del?id='.$sms['id']);?>">删除</a>
                        </td>
