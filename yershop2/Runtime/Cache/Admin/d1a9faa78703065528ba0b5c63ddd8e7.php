...
<body>
 ...
 <?php if(isset($cate_rows)): ?></div><?php endif; ?> 
	...
	<div class="main-title">
		<h2>广告管理 [ <?php if(isset($_GET['group'])): ?><a href="<?php echo U('index');?>">全部</a><?php else: ?><strong>全部</strong><?php endif; ?>&nbsp;<?php if(is_array($group)): foreach($group as $key=>$vo): if(($group_id) != $key): ?><a href="<?php echo U('index?group='.$key);?>"><?php echo ($vo); ?></a><?php else: ?><strong><?php echo ($vo); ?></strong><?php endif; ?>&nbsp;<?php endforeach; endif; ?> ]</h2>
	</div>

	<div class="cf">
		<a class="btn" href="<?php echo U('add');?>">新 增</a>
		  <button class="btn ajax-post confirm" url="<?php echo U('del');?>" target-form="ids">删 除</button>
         <button class="btn ajax-post" target-form="ids" url="<?php echo U("Ad/setStatus",array("status"=>1));?>">启 用</button>
		 <button class="btn ajax-post" target-form="ids" url="<?php echo U("Ad/setStatus",array("status"=>0));?>">禁 用</button>（位置：1-首页，2-频道页， 3-内容页) <!-- 高级搜索 -->
        <div class="search-form fr cf">
            <div class="sleft">
                <input type="text" name="title" class="search-input" value="<?php echo I('title');?>" placeholder="请输入推广图名称">
                <a class="sch-btn" href="javascript:;" id="search" url="/yershop2/index.php?s=/Admin/Ad/index.html"><i class="btn-search"></i></a>
            </div>
        </div>
	</div>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					<td><input class="ids row-selected" type="checkbox" name="ids[]" value="<?php echo ($vo["id"]); ?>"></td>
                        ...
                        <td><?php echo ($vo["ypid"]); ?></td>
					 <td><?php echo (get_cate($vo["ypid"])); ?></td>
						
                       <td><?php echo ($vo["url"]); ?></td>  <td><?php echo ($vo["status"]); ?></td>
