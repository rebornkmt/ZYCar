...
<body>
 ...
 <?php if(isset($cate_rows)): ?></div><?php endif; ?> 
	...
	<div class="main-title">
		<h2>留言管理 [ <?php if(isset($_GET['group'])): ?><a href="<?php echo U('index');?>">全部</a><?php else: ?><strong>全部</strong><?php endif; ?>&nbsp;<?php if(is_array($group)): foreach($group as $key=>$vo): if(($group_id) != $key): ?><a href="<?php echo U('index?group='.$key);?>"><?php echo ($vo); ?></a><?php else: ?><strong><?php echo ($vo); ?></strong><?php endif; ?>&nbsp;<?php endforeach; endif; ?> ]</h2>
	</div>
	...
	<div class="cf">
        ...
        <div class="search-form fr cf">
            <div class="sleft">
                <input type="text" name="title" class="search-input" value="<?php echo I('title');?>" placeholder="手机">
                <a class="sch-btn" href="javascript:;" id="search" url="/yershop2/index.php?s=/Admin/Message/index.html"><i class="btn-search"></i></a>
            </div>
        </div>
	</div>
	...
	<div class="data-table table-striped">
		 <table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$message): $mod = ($i % 2 );++$i;?><tr>
					<td><input class="ids row-selected" type="checkbox" name="id[]" value="<?php echo ($message["id"]); ?>"></td>
						<td><?php echo ($message["id"]); ?></td>
							<td><?php echo ($message["goodid"]); ?></td>
						<td><a title="编辑" href="<?php echo U('edit?id='.$message['id']);?>"><?php echo (get_regname($message["uid"])); ?></a></td>
						 
						 <td><?php echo ($message["uid"]); ?></td>
						 <td><?php echo ($message["content"]); ?></td>
						  <td><?php echo ((isset($message["group"]) && ($message["group"] !== ""))?($message["group"]):"无"); ?></td>
					   <td><?php echo ($message["status"]); ?></td>
                         <td><?php echo (date('Y-m-d H:i:s',$message["create_time"])); ?></td>
					<td>
                              <a title="编辑" href="<?php echo U('edit?id='.$message['id']);?>">编辑</a>
							  <a title="编辑" href="<?php echo U('reply?id='.$message['id']);?>">回复</a>
                            <a class="confirm ajax-get" title="删除" href="<?php echo U('del?id='.$message['id']);?>">删除</a>
                        </td>
