 ...
 <?php if(isset($cate_rows)): ?></div><?php endif; ?> 
       ...
       分组解释：  1-表示个人，2-表示所有人<!-- 高级搜索 -->
        <div class="search-form fr cf">
            <div class="sleft">
                ...
                <a class="sch-btn" href="javascript:;" id="search" url="/yershop2/index.php?s=/Admin/Envelope/index.html"><i class="btn-search"></i></a>
            </div>
        </div>
	</div>
	...
	<div class="data-table table-striped">
		 <table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$envelope): $mod = ($i % 2 );++$i;?><tr>
					<td><input class="ids row-selected" type="checkbox" name="id[]" value="<?php echo ((isset($envelope["id"]) && ($envelope["id"] !== ""))?($envelope["id"]):"无"); ?>"></td>
						<td><?php echo ((isset($envelope["id"]) && ($envelope["id"] !== ""))?($envelope["id"]):"无"); ?></td>
						<td><a title="编辑" href="<?php echo U('edit?id='.$envelope['id']);?>"><?php echo ((isset($envelope["title"]) && ($envelope["title"] !== ""))?($envelope["title"]):"无"); ?></a></td>
						 
						 <td><?php echo ($envelope["username"]); ?></td>
						  <td><?php echo ($envelope["view"]); ?></td>
						 <td><?php echo ((isset($envelope["group"]) && ($envelope["group"] !== ""))?($envelope["group"]):"无"); ?></td>	 
					   <td><?php echo ((isset($envelope["status"]) && ($envelope["status"] !== ""))?($envelope["status"]):"无"); ?></td>
                         <td><?php echo (date('Y-m-d H:i:s',$envelope["create_time"])); ?></td>
					<td>
                            <a title="编辑" href="<?php echo U('edit?id='.$envelope['id']);?>">编辑</a>
                            <a class="confirm ajax-get" title="删除" href="<?php echo U('del?id='.$envelope['id']);?>">删除</a>
                        </td>
